...
from vitadex.constitution import Constitution, ConstitutionMissingError
...
def _conn():
    ...
    ignore_constitution = os.getenv("VITADEX_IGNORE_CONSTITUTION") or os.getenv("PRIVATE_OS_IGNORE_CONSTITUTION") or "false"
    if ignore_constitution.lower() not in {"1", "true", "yes"}:
        try:
...
def init() -> None:
    ...
    print(f"[green]VitaDex initialized[/green]: {settings.db_path}")
